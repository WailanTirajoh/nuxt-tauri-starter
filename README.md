...
```ts
import { useBarcodeScanner, Format } from "@/composables/useBarcodeScanner"

const { scan, scanning, result, isSupported } = useBarcodeScanner()

const code = await scan({ formats: [Format.QRCode] })
// code?.content  → the decoded string (or null if it failed/was cancelled)
...
> Mobile only — on desktop `isSupported` is `false` and `scan()` resolves to `null`, routing a clear "mobile-only" error to `onError`. The plugin adds the camera permission on Android automatically; for iOS the camera usage string lives in `src-tauri/Info.ios.plist`. Permissions are scoped to mobile in `src-tauri/capabilities/mobile.json`.
